...
public class Patch implements Cloneable, Comparable<Patch> {
	...
	public Map<String,Long> getAgesOfInfestation() {
		...
		while(it.hasNext()){
			String key = it.next();
			agesOfInfestation.put(key, occupants.get(key).getAgeOfInfestation());
		}
	...
	public Map<String,Long> getCumulativeAgesOfInfestation() {
		...
		while(it.hasNext()){
			String key = it.next();
			cumulativeAgesOfInfestation.put(key, occupants.get(key).getCumulativeAgeOfInfestation());
		}
	...
	public Occupancy getOccupant(String key){return occupants.get(key);}

	public Map<String,Occupancy> getOccupants(){return occupants;}
	...
	public boolean hasOccupant(String key){return occupants.containsKey(key);}
	
	public void incrementControlTime(String control, long increment) {
		Iterator<String> it = occupants.keySet().iterator();
		while(it.hasNext()){
			Occupancy occ = occupants.get(it.next());
			occ.getControls().put(control, occ.getControls().get(control)+increment);
		}
	}
